...
- [哪些特性让你觉得 SVN 比 Git 更好用？](https://www.zhihu.com/question/267292008)
- [为什么大部分游戏公司仍在坚持使用SVN？](https://www.zhihu.com/question/590727955)
...
- 恶意碰撞
    - 例如，原文件`Alice 需要偿还对 Bob 的 1000 美元的债务` 有一个哈希值x
    - 我们希望铲产生一个文件, 同样得到哈希值x, 之前提到这样的文件有无数多个
    - 可是有意义的恶意碰撞应该是类似`Alice 需要偿还对 Bob 的 10 美元的债务`的文件
    	- 碰撞文件应该是内容类似, 大小相近的文件才能蒙混过关
    - 之前提到稍有不同的输入(例如两个文件只差一个比特)，给出的哈希值是完全不同的
    - 这意味着两个大小相近的文件很难碰撞, 即使恶意碰撞，恶意文件也很难是有意义的
